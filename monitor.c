...
#define FLAG_FMT(f, ch)                                                        \
    (status & FLAG_##f ? TXT_B3 : TXT_LO),                                     \
        (status & FLAG_##f ? ch : tolower(ch))

const char *_monitor_names[] = {"read", "write", "data", "execute", "x", 0};

const int _monitor_vals[] = {MONITOR_READ, MONITOR_WRITE, MONITOR_DATA,
...
char *prompt() {
    sprintf(_prompt,
            TXT_LO "PC" TXT_N " %.4x  "
                   ...
                   "  " TXT_LO "A " TXT_N "%.2x " TXT_LO "X " TXT_N
                   "%.2x " TXT_LO "Y " TXT_N "%.2x " TXT_LO "SP " TXT_N
                   "%.2x " TXT_LO "> " TXT_N,
            pc, FLAG_FMT(SIGN, 'N'), FLAG_FMT(OVERFLOW, 'V'),
            /* - */ FLAG_FMT(BREAK, 'B'), FLAG_FMT(DECIMAL, 'D'),
            FLAG_FMT(INTERRUPT, 'I'), FLAG_FMT(ZERO, 'Z'), FLAG_FMT(CARRY, 'C'),
            a, x, y, sp);
    return _prompt;
}
...
void clear_heatmap(uint16_t start, uint16_t end, int mode) {
    ...
    for (addr = start; addr < endl; addr++) {
        if (mode & MONITOR_READ)
            heat_rs[addr] = 0;
        if (mode & MONITOR_WRITE)
            heat_ws[addr] = 0;
        if (mode & MONITOR_PC)
            heat_xs[addr] = 0;
    }
...
int compare_hotspots(const void *a, const void *b) {
...
void show_heatmap(uint16_t start, uint16_t end, int mode) {
    ...
    for (zoom = 0; /**/; zoom += 2)
        if (((start >> zoom) << zoom) + (1024 << zoom) >= endl)
            break;
    ...
    /* we aggregate by max to preserve the same scale as we zoom
        in some cases sum might be more useful but for max is better for finding
       hotspots */
    for (i = 0; i < 1024; i++)
        data[i] = 0;
    for (i = 0; i < 0x10000; i++)
        heat[i] = 0;
    for (addr = start; addr < start + (1024 << zoom) && addr < 0x10000;
         addr++) {
        heat[addr] = ((mode & MONITOR_READ ? heat_rs[addr] : 0) +
    ...
    */
    ...
    for (dmax = i = 0; i < 1024; i++)
        if (data[i] > dmax)
            dmax = data[i];
    /* now calculate k by dividing bit length of largest value by 7 and
     * rounding: */
    heat_scale = (int)(bitlen(dmax) / 7.0 + 0.5);
    if (!heat_scale)
        heat_scale =
            1; /* want at least one bit per bucket or labels looks silly */

    /* add a header line.  with even zoom levels each row of 64 blocks shares
     * the same labeling */
    if (zoom & 2)
    ...
    /* add a legend */
    printf("\n%c%c%c count 0", mode & MONITOR_READ ? 'r' : '-',
           mode & MONITOR_WRITE ? 'w' : '-', mode & MONITOR_PC ? 'x' : '-');
    for (i = 0; i < 8; i++) {
        d = 1 << (i * heat_scale);
    ...
    if (symbols) {
        ...
        for (n = 0, i = 0; i < 0x10000; i++) {
            if (hotspots[i] && heat[i]) {
                /* count symbols with heat */
                n++;
            } else {
                /* zero out unlabeled addresses */
                heat[i] = 0;
            }
            hotspots[i] = i;
        }
        /* any hot spots to display? */
        if (n == 0)
            return;

        qsort(hotspots, 0x10000, sizeof(uint16_t), compare_hotspots);
...
uint16_t disasm(uint16_t start, uint16_t end) {
    ...
    */
    for (addr = start; addr < endl; /**/) {
        ...
        p = line;
        p += sprintf(line, "%c%c %.4x  " TXT_LO, pc == addr ? '*' : ' ',
                     breakpoints[addr] & MONITOR_PC ? 'B' : ' ', addr);
        ...
        *p = ' ';
        ...
        if (n == 1) {
            ...
            if (n == 3) {
                p += sprintf(p, fmt, _fmt_addr(buf + 16, memory[addr], 2),
                             _fmt_addr(buf, addr + 2 + offset, 4), offset);
                addr += 2;
            } else {
                p += sprintf(p, fmt, _fmt_addr(buf, addr + 1 + offset, 4),
                             offset);
                addr++;
            }
        } else if (n == 2) {
            ...
            p += sprintf(p, fmt,
                         _fmt_addr(buf, *(uint16_t *)(memory + addr), 4));
            addr += 2;
        }
        puts(line);
    }
...
void cmd_go() {
    ...
    if (E_OK != parse_addr(&pc, pc) || E_OK != parse_end())
        return;
...
void cmd_continue() {
    ...
    uint16_t addr;
    if (E_OK != parse_addr(&addr, pc) || E_OK != parse_end())
        return;

    if (addr != pc && !(breakpoints[addr] & MONITOR_PC))
        breakpoints[addr] |= MONITOR_PC | MONITOR_ONCE;
    step_mode = STEP_RUN;
}
...
void _cmd_single(int mode) {
    ...
    step_mode = mode;
    if (E_OK != parse_int(&v, 1) || E_OK != parse_end())
        return;
    step_target = v;
    if (step_target > 1)
        puts("...");
...
void cmd_call() {
    ...
    if (E_OK != parse_addr(&target, DEFAULT_REQUIRED) || E_OK != parse_end())
        return;

    /* trigger break once we return from subroutine */
    pc = target;
    if (!(breakpoints[ret] & MONITOR_PC))
        breakpoints[ret] |= MONITOR_PC | MONITOR_ONCE;
...
void cmd_signal() {
    ...
    if (E_OK != parse_enum(_names, _vals, &v, DEFAULT_REQUIRED) ||
        E_OK != parse_end())
        return;
...
void cmd_disasm() {
    ...
    if (E_OK != parse_range(&start, &end, org, 48) || E_OK != parse_end())
        return;
...
void cmd_memory() {
    ...
    if (E_OK != parse_range(&start, &end, org, 64) || E_OK != parse_end())
        return;
...
void cmd_break() {
    ...
    if (E_OK != parse_range(&start, &end, pc, 1))
        return;
    endl = end < start ? 0x10000 : end;

    if (E_OK != parse_enum(_monitor_names, _monitor_vals, &mode, MONITOR_PC) ||
        E_OK != parse_end())
        return;
...
void cmd_inspect() {
    ...
    if (E_OK != parse_range(&start, &end, 0, 0) ||
        E_OK != parse_addr(&nmax, 16) || E_OK != parse_end())
        return;

    endl = end <= start ? 0x10000 : end;

    for (prv = n = 0, addr = start; addr < endl; prv = brk, addr++) {
        if (addr == start || heat_rs[addr] > rmax)
            rmax = heat_rs[raddr = addr];
        if (addr == start || heat_ws[addr] > wmax)
            wmax = heat_ws[waddr = addr];
        if (addr == start || heat_xs[addr] > xmax)
            xmax = heat_xs[xaddr = addr];
        if (n >= nmax)
            continue;
        ...
        if (new) { /* did some flags just switch on? */
            ...
            if (new &MONITOR_PC) {
                for (span = addr;
                     span < 0x10000 && breakpoints[span] & MONITOR_PC;
                     span++) /**/
                    ;
                if (--span != addr)
                    printf("  x %.4x.%.4x", addr, span);
                else
                    printf("  x %.4x", addr);
            }
            if (new &MONITOR_READ) {
                for (span = addr;
                     span < 0x10000 && breakpoints[span] & MONITOR_READ;
                     span++) /**/
                    ;
                if (--span != addr)
                    printf("  r %.4x.%.4x", addr, span);
                else
                    printf("  r %.4x", addr);
            }
            if (new &MONITOR_WRITE) {
                for (span = addr;
                     span < 0x10000 && breakpoints[span] & MONITOR_WRITE;
                     span++) /**/
                    ;
                if (--span != addr)
                    printf("  w %.4x.%.4x", addr, span);
                else
                    printf("  w %.4x", addr);
            }
            puts("");
    ...
    if (n >= nmax)
        ...
        puts("(no labels or breakpoints found)");
    printf("hotspots: $%" PRIx64 " read @ %.4x; $%" PRIx64
           " write @ %.4x; $%" PRIx64 " execute @ %.4x\n",
...
void cmd_set() {
    ...
    name = parse_delim();
    if (!name) {
        puts("Missing register/flag, expected one of a,x,y,sp,pc or "
             "n,v,b,d,i,z,c");
        return;
    }
    if (E_OK != parse_int(&v, DEFAULT_REQUIRED) || E_OK != parse_end())
        return;
    if (E_OK != set_reg_or_flag(name, v))
        puts("Unknown register/flag, expected one of a,x,y,sp,pc or "
             "n,v,b,d,i,z,c");
}
...
void cmd_fill() {
    ...
    if (E_OK != parse_range(&start, &end, DEFAULT_REQUIRED, 1))
        return;
...
void cmd_convert() {
    ...
    for (n = 0;; n++) {
        ...
        if (E_OK != err)
            return;
        printf("%.*s\t:=  $%x  #%d  %%%s", parsed_length(), parsed_str(), v, v,
               bitstr(v));
        if (32 <= v && v < 127)
            printf("  '%c", v);
        puts("");
...
void cmd_unlabel() {
    ...
    if (E_OK != parse_addr(&addr, DEFAULT_REQUIRED) || E_OK != parse_end())
        return;
...
void cmd_save() {
    ...
    if (E_OK != parse_range(&start, &end, 0, 0) || E_OK != parse_end())
        return;
...
void cmd_heatmap() {
    ...
    if (err != E_OK && err != E_MISSING)
        return;
    if (cmd == 2 && !(fname = parse_delim())) {
        puts("Missing filename");
        return;
    }
    /* try parsing a mode before range in case range was omitted, otherwise
     * 'heat x' interprets x as an expression defining the range... */
    if (E_OK !=
        parse_enum(_monitor_names, _monitor_vals, &mode, DEFAULT_OPTIONAL)) {
        if (E_OK != parse_range(&start, &end, 0, 0))
            return;
        if (E_OK !=
            parse_enum(_monitor_names, _monitor_vals, &mode, MONITOR_DATA))
            return;
    }

    switch (cmd) {
    case 0:
        show_heatmap(start, end, mode);
        break;
    case 1:
        clear_heatmap(start, end, mode);
        break;
    case 2:
        (void)save_heatmap(fname, start, end, mode);
        break;
    }
}
...
void cmd_help() {
    int i;
    puts("\n"
         "Commands can be shortened to their first few letters, searched in "
         "the order below.\n"
         "For example 'd' means 'disassemble' while 'de' becomes 'delete'.  "
         "Use <enter> to\n"
         "repeat commands like step, next, dis or mem and advance through "
         "memory.  Command\n"
         "history (up/down arrow) and tab completion are also available.\n"
         "\n"
         "Literal values like 1234 are assumed to be hex unless explicitly "
         "prefixed as binary (%),\n"
         "decimal (#), hex ($) or ascii (').  Use the ~ command as a "
         "calculator to display\n"
         "values or expressions in all bases. C-style expressions can be used "
         "for most\n"
         "address and value arguments.  Expressions can include "
         "(case-sensitive) labels\n"
         "as well as CPU registers A, X, Y, PC, SP and flags N, V, B, D, I, Z "
         "and C.\n"
         "The usual operators are available, along with * and @ to deference "
         "memory by byte or\n"
         "word and unary < and > to extract the lo and high byte of their "
         "argument.\n"
         "For example >(@(*(pc+1))+x) takes the high byte of an indirect zp "
         "address.\n"
         "\n"
         "Write address ranges as start.end or start..offset, where the .. "
         "form is a shortcut\n"
         "for start.start+offset.   A few examples: 1234.1256; pc .. 10; "
         ".label; and ..20.\n"
         "\n"
         "Available commands:\n");
...
Command _cmds[] = {
    {"go", "[addr] - run from pc (or optional addr) until breakpoint", 1,
     cmd_go},
    {"continue [addr]", "- run from pc until breakpoint (or optional addr)", 1,
     cmd_continue},
    {"step", "- [count] step by single instructions", 1, cmd_step},
    {"next", "- [count] like step but treats jsr ... rts as one step", 1,
     cmd_next},
    {"call", "addr - call subroutine leaving PC unchanged", 0, cmd_call},
    {"signal", "irq|nmi|reset - signal an interrupt", 0, cmd_signal},

    {"disassemble", "[range] - show code disassembly for range (or current)", 1,
     cmd_disasm},
    {"memory", "[range] - dump memory contents for range (or current)", 1,
     cmd_memory},
    {"stack", "- show stack contents, sp+1 through $1ff", 0, cmd_stack},
    {"break",
     "[range] [r|w|d|x] - trigger break on read, write, any access or execute "
     "(default)",
     0, cmd_break},
    {"delete", "[range] - remove all breakpoints in range (default PC)", 0,
     cmd_delete},
    {"inspect",
     "[range] [max] - show labels and breakpoints in range, up to max lines", 0,
     cmd_inspect},

    {"set", "{a|x|y|sp|pc|n|v|d|i|z|c} value - modify a register or flag", 0,
     cmd_set},
    {"ticks", "[value] - query or set the current cycle count", 0, cmd_ticks},
    {"fill", "range value ... - set memory contents in range to value(s)", 0,
     cmd_fill},
    {"~", "value ... - show each value in binary, octal, hex and decimal", 0,
     cmd_convert},
    ...
    {"save", "romfile [range] - write memory to file (default full dump)", 0,
     cmd_save},
    {"heatmap",
     " [clear|save mapfile] [range] [r|w|d|x] - view, reset or save heatmap "
     "data",
     0, cmd_heatmap},
    {"blockfile",
     "[blockfile] - use binary file for block storage, empty to disable", 0,
     cmd_blockfile},
...
void monitor_init(const char *labelfile) {
    ...
    if (labelfile) {
        ...
        */
        ...
        if (!quiet)
            printf("Imported %d labels from %s.  Skipped %d lines (locals or "
                   "malformed).\n",
                   ok, labelfile, skip);
        fclose(f);
    }
...
void monitor_exit() { linenoiseHistorySave(".c65"); }
...
void monitor_command() {
    ...
    if (step_mode != STEP_NONE) {
        ...
        if (break_flag & MONITOR_DATA) {
            printf("%.4x: memory %s\n", rw_brk,
                   break_flag & MONITOR_READ ? "read" : "write");
